...
def migrate_from_s3(max: nil, limit: nil)
  ...
  puts "Migrating up to #{max} of #{limit} posts..." if max
  ...
  migrate_posts.find_each do |post|
    begin
      updated = false
      post_tag = "#{post.topic_id}/#{post.post_number} - https://#{GlobalSetting.hostname}#{post.url}"
      ...
      post.raw.gsub!(/(upload:\/\/[0-9a-zA-Z]+\.\w+)/) do |url|
        begin
          ...
          puts "Not migrating url in #{post_tag} due to exception #{e}"
          url
        end
      end
